...
/*
* Read the file with name path and returns bytes read
*/
...
/*
 * Delete the file with name path from filesystem
 */
static int my_unlink(const char *path){
    ...
    char* filename = (char*)(path+1); //Con esto quitamos la barra '/'

    //Buscamos el fichero
    if( (idNode=findFileByName(&myFileSystem, filename)) == -1){
        return -ENOENT;
    }

    //Modificamos el tamaño a 0
    if(resizeNode(idNode, 0)<0)
        return -EIO;

    myFileSystem.directory.files[idNode].freeFile = true; //liberar el nodo correspondiente al fichero
    myFileSystem.directory.numFiles--;//Disminuir número de ficheros en el directorio
    myFileSystem.numFreeNodes++;//Aumentar el número de nodos libres

    myFileSystem.nodes[idNode]->freeNode = true;

    updateDirectory(&myFileSystem);
    updateNode(&myFileSystem, idNode, myFileSystem.nodes[idNode]);

    free(myFileSystem.nodes[idNode]);
    myFileSystem.nodes[idNode]=NULL;
