...
{	stHeaderEntry* array = NULL;
	...
	if(fread(&nr_files, sizeof(int), 1, tarFile) == 0){
		printf("Error reading the number of files\n");
	}
...
{	FILE * tar;
	...
	if((tar = fopen(tarName, "w")) == NULL){
		printf("Error trying to open the tar file\n");
		return EXIT_FAILURE;
	}
	...
	if(fseek(tar, offData, SEEK_SET) < 0){
		printf("Error in function fseek!\n");
		return EXIT_FAILURE;
	}
	...
	for(i=0; i < nFiles; i++){
		...
		if((inputFile = fopen(fileNames[i], "r")) == NULL){
			printf("Error reading this file %s", fileNames[i]);
			fclose(tar);
			return EXIT_FAILURE;
		}
	...
	if(fwrite(&nFiles, sizeof(int), 1, tar) < 0){
		printf("Error writing in tarFile\n");
		return EXIT_FAILURE;
	}
...
{	FILE * tar;
	...
	if(array == NULL){
		printf("Error reading the tarball's header\n");
		return EXIT_FAILURE;
	}
	...
	for(i = 0; i < nFiles; i++){
		...
		if((wBytes = copynFile(tar, outputFile, array[i].size)) != array[i].size){
			printf("Error writing in %s file\n", name);
			return EXIT_FAILURE;
		}
		else{
